...
RUN apt-get update \
    && apt-get install -y --no-install-recommends bash netcat-openbsd procps python3 python3-pip tini \
    && rm -rf /var/lib/apt/lists/*

COPY assets/spark-${SPARK_VERSION}-bin-hadoop3.tgz /tmp/spark.tgz
